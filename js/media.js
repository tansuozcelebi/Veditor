...
import { uid, IMAGE_DEFAULT_DURATION } from './state.js';
...
function once(el, ev, { timeout = 15000, errorEvent = 'error' } = {}) {
...
async function ensureFiniteDuration(el) {
...
export async function loadMedia(file) {
  ...
  const m = { id: uid(), name: file.name, kind, file, url, size: file.size, type: file.type, duration: 0, width: 0, height: 0, thumbnails: [], peaks: null, poster: null, analyzing: true };
  if (kind === 'image') {
    const img = new Image();
    img.src = url;
    await once(img, 'load');
...
export async function generateThumbnails(m, onProgress) {
...
export async function generatePeaks(m) {
...
export async function importFiles(store, files, { onMedia, onError } = {}) {
  const added = [];
